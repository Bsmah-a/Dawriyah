...
public struct WheelView<Data, ID, Label>: View where Data: RandomAccessCollection, ID: Hashable, Label: View  {
   ...
   // @Binding private var items: [String]
    ...
    @State private var textfieldt: String = ""
    ...
    public var body: some View {
       // Image("wheeltitle").resizable().frame(width: 350, height: 90).offset(x: 0, y: -200)
        ...
        GeometryReader { geometry in
            ...
            VStack{
                Image("wheeltitle").resizable().frame(width: 350, height: 90).offset(x: -5, y: -170)
                
//                TextField(" اكتب تشويقة عن سالفتك:..", text: $textfieldt).padding(.leading,160).background(Color.gray.opacity(0.2).cornerRadius(8)).offset(x: 0, y: 320)
//                Button(action: {
//                    //
//                },label: {
//                    Text("احفظها !").padding().frame(maxWidth: .infinity).background(Color("Color2")).cornerRadius(20).foregroundColor(.white).offset(x:0, y:350)
//                })
            }
            .padding()
...
struct WheelView_Previews: PreviewProvider {
    static let items: [String] = ["سالفه١", "سالفه٢", "سالفه٣", "سالفه٤", "سالفه٥", "سالفه٦","ll","h", "l"]
